...
function markdownImageFromSource(source: string): {
  src: string
  alt: string
} | null {
  const parsed = parseMarkdownImageSource(source)
  if (!parsed) return null
  return { alt: parsed.alt, src: parsed.rawSrc }
}
      ...
      enter(node) {
        ...
        switch (node.name) {
          case 'Image': {
            ...
            const parsed = markdownImageFromSource(source)
            if (parsed) {
              ranges.push({
                from: node.from,
                to: node.to,
                deco: Decoration.replace({
                  widget: new ImageWidget(parsed.src, parsed.alt, node.from),
                }),
              })
              return false
            }
