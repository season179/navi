// Claw IM sidebar echoing Kun's SidebarClaw.tsx
// (../Kun/src/renderer/src/components/chat/SidebarClaw.tsx).
// Visual only: mock channel snapshots and preview modes.
...
export type ClawImConversationSnapshot = {
  chatId: string
  senderName: string
...
export type ClawImChannelSidebarSnapshot = {
  id: string
  ...
  model: string
  threadId: string
  conversations: ClawImConversationSnapshot[]
  updatedAt: string
}
...
export function ClawProviderPill({
  ...
  return (
    <span
      className={`claw-provider-pill${active ? ' is-active' : ''}`}
    >
      <ClawProviderLogo provider={provider} className="claw-provider-pill-logo" />
    </span>
  )
}
...
function SidebarIconButton({
  ...
  return (
    <button
      type="button"
      className="claw-sidebar-icon-btn"
      title={title}
      aria-label={title}
      disabled={disabled}
      onPointerDown={(event) => {
...
function SidebarTreeRow({
  ...
  return (
    ...
    >
      <button
        type="button"
        className="claw-sidebar-tree-row-btn"
        onClick={onClick}
        disabled={disabled}
      >
        {children}
      </button>
      {trailing ? <div className="claw-sidebar-tree-row-trailing">{trailing}</div> : null}
      {actions ? <div className="claw-sidebar-tree-row-actions">{actions}</div> : null}
    </div>
  )
}
    ...
    <div className="claw-sidebar-content">
      <div className="claw-sidebar-section-header">
        ...
        <div className="claw-sidebar-section-actions">
          ...
          >
            <Settings className="claw-sidebar-icon" strokeWidth={1.75} />
          </SidebarIconButton>
        </div>
      </div>
      ...
      <div className="claw-sidebar-scroll">
        {channels.length === 0 ? (
          ...
          <div className="claw-sidebar-channel-list">
            {sortedChannels.map((channel) => {
              ...
              const running = sortedConversations.some(
                (conversation) => conversation.localThreadId.trim() === (activeThreadId ?? '').trim(),
              )
              ...
              return (
                <div key={channel.id} className="claw-sidebar-channel-item">
                  <SidebarTreeRow
                    active={active}
                    dimmed={disabled}
                    ...
                    actions={
                      ...
                      >
                        <RefreshCw className="claw-sidebar-icon" strokeWidth={1.9} />
                      </SidebarIconButton>
                    }
                  >
                    <MessageSquare className="claw-sidebar-channel-icon" strokeWidth={1.8} />
                    <ClawProviderPill provider={channel.provider} active={active} />
                    <span className="claw-sidebar-channel-copy">
                      <span className="claw-sidebar-channel-title">{channel.label}</span>
                      <span className="claw-sidebar-channel-sub">{secondaryLabel}</span>
                    </span>
                  </SidebarTreeRow>
                </div>
              )
...
function previewSnapshot(mode: ClawSidebarPreviewMode): {
  channels: ClawImChannelSidebarSnapshot[]
  activeChannelId: string
  activeThreadId: string | null
} {
  if (mode === 'empty') {
    return { channels: [], activeChannelId: '', activeThreadId: null }
  }
  if (mode === 'disabled') {
    return {
      channels: PREVIEW_CHANNELS_DISABLED,
      activeChannelId: 'feishu-team',
      activeThreadId: null,
    }
  }
  if (mode === 'running') {
    return {
      ...
      activeChannelId: 'feishu-team',
      activeThreadId: 'thread-feishu-1',
    }
  }
  return {
    channels: PREVIEW_CHANNELS,
...
export function ClawSidebarPreview({ mode }: { mode: ClawSidebarPreviewMode }): ReactElement {
  const initial = useMemo(() => previewSnapshot(mode), [mode])
  const [activeChannelId, setActiveChannelId] = useState(initial.activeChannelId)

  return (
    <div className="claw-sidebar-preview-wrap">
      <aside className="claw-sidebar-shell">
        <ClawSidebarContent
