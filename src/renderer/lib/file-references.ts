// Path detection + deepseek-file: href helpers ported from Kun
// (../Kun/src/renderer/src/lib/file-references.ts). Visual-only in navi:
// used for markdown link styling, not workspace file preview/open.
...
const LINE_SUFFIX =
  /(?::(\d+)(?::(\d+))?|#L(\d+)(?:-L\d+)?|\s*[（(](?:line|lines)\s+(\d+)[）)]|\s*[（(]第\s*(\d+)\s*行[）)]|\s+line\s+(\d+)|\s+第\s*(\d+)\s*行)/iy
