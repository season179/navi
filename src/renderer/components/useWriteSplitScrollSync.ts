// Split-pane scroll sync echoing Kun's use-write-split-scroll-sync
// (../Kun/src/renderer/src/components/write/use-write-split-scroll-sync.ts).
// Keeps the CodeMirror editor and markdown preview aligned in split mode.
...
export function useWriteSplitScrollSync({
  ...
  useEffect(() => {
    ...
    const editor =
      editorRootRef.current?.querySelector<HTMLElement>('.cm-scroller') ?? null
