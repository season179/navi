...
function HomePage() {
  ...
  // Visual preview for the ported ComposerImageAttachmentPreview row
  // (?composerAttachmentsPreview=1|noPreview).
  const composerAttachmentsPreview = useMemo(() => {
    if (typeof window === 'undefined') return undefined
    const params = new URLSearchParams(window.location.search)
    if (!params.has('composerAttachmentsPreview')) return undefined
    const value = params.get('composerAttachmentsPreview')
    const mode: ComposerAttachmentsPreviewMode =
      value === 'noPreview' ? 'noPreview' : 'default'
    return resolveComposerAttachmentsPreview(mode)
  }, [])
  ...
  // (?writeWorkspaceToolbar=1|pdf|dirty|saving|error|readonly|exportMenu|modeMenu|assistant|review|image|exporting|rich|source|split|preview|live|liveDisabled).
  ...
  // (?writeInlineAgent=1|blockMenu|emptyAgents|askOnly|preferAbove|inFlight|skills|imageMode).
  ...
  // (?sddDraftEditorViewPreview=1|dirty|saving|error|upgrading|designContext|noDraft|assistantOpen|assistantTimeline|assistantBusy|leftCollapsed|withNotice|inlineAgent|richFallback).
  const sddDraftEditorViewPreviewMode = useMemo((): SddDraftEditorViewPreviewMode | null => {
    ...
    if (value === 'assistantBusy') return 'assistantBusy'
  ...
  // Visual preview for the ported WriteHtmlEmbed
  // (?writeHtmlEmbed=1|loaded|error|missing).
  const writeHtmlEmbedPreviewMode = useMemo((): WriteHtmlEmbedPreviewMode | null => {
    if (typeof window === 'undefined') return null
    const params = new URLSearchParams(window.location.search)
    if (!params.has('writeHtmlEmbed')) return null
    const value = params.get('writeHtmlEmbed')
  ...
  if (writeHtmlEmbedPreviewMode) {
    return <WriteHtmlEmbedPreview mode={writeHtmlEmbedPreviewMode} />
  }
