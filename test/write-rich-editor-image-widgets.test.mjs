...
test('resolveWriteRichEditorPreviewMode routes image widget preview params', () => {
  ...
  assert.equal(resolveWriteRichEditorPreviewMode('htmlEmbedError'), 'htmlEmbedError')
  assert.equal(resolveWriteRichEditorPreviewMode('htmlEmbedMissing'), 'htmlEmbedMissing')
...
test('isWriteRichEditorImageWidgetMode recognizes image widget modes', () => {
  ...
  assert.equal(isWriteRichEditorImageWidgetMode('htmlEmbedError'), true)
  assert.equal(isWriteRichEditorImageWidgetMode('htmlEmbedMissing'), true)
...
test('imageWidgetSnapshotForMode returns html embed error and missing chrome', () => {
  const error = imageWidgetSnapshotForMode('htmlEmbedError')
  assert.equal(error.htmlEmbed?.visualState, 'error')
  const missing = imageWidgetSnapshotForMode('htmlEmbedMissing')
  assert.equal(missing.htmlEmbed?.visualState, 'missing')
})
