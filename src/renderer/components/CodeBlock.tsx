...
type MarkdownNode = { position?: MarkdownPosition }
...
function CodeBlockBody({ code, language }: { code: string; language: string }): ReactNode {
  ...
  // Show the plain fallback immediately, then swap in highlighted HTML once
  // shiki resolves (async). Cancel on unmount / input change.
  useEffect(() => {
    let cancelled = false
  ...
  return (
    <div className="ds-code-block" data-language={language} data-streamdown="code-block">
      <div className="ds-code-block-header" data-streamdown="code-block-header">
        ...
        <div className="ds-code-block-actions">
          ...
          >
            <Download className="ds-code-block-icon" strokeWidth={1.9} />
          </button>
          <button
            type="button"
            className="ds-code-block-action"
          ...
          >
            {isCopied ? (
              <Check className="ds-code-block-icon" strokeWidth={2.1} />
            ) : (
              <Copy className="ds-code-block-icon" strokeWidth={1.9} />
            )}
          </button>
          {expandable ? (
            <button
            ...
            >
              {expanded ? (
                <ChevronUp className="ds-code-block-icon" strokeWidth={1.9} />
              ) : (
                <ChevronDown className="ds-code-block-icon" strokeWidth={1.9} />
              )}
            </button>
          ) : null}
        </div>
      ...
      <div className={`ds-code-block-body ${expandable && !expanded ? 'is-collapsed' : ''}`}>
        <div
          ...
          // Trusted: HTML is produced locally by shiki / our escaped fallback.
...
function CodeComponent({ node, className, children, ...props }: CodeProps) {
  ...
  return <CodeBlockBody code={text} language={language} />
}
...
MemoCode.displayName = 'CodeBlock'

export { MemoCode as CodeBlock }
